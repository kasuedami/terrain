...
pub fn generate(size: Vec3, heihtmap: Handle<Image>) -> Mesh {
    // extract some values from the parameters for better readabillity
    let total_width = size.x;
    let total_length = size.z;
    let half_width = total_width / 2.0;
    let half_length = total_length / 2.0;
    let max_height = size.y;

    // extract heightmap resolutionretreive the heightmap image and extract its size
